...
%%%%%%%%%%%%%%%%%%%%%%
%v0.1/03/10/2018
%Performs semi-automatic segmentation and isolated (ROIRED) ie. cells of
%particular interest 
...
default_pathname='F:\.shortcut-targets-by-id\1hAipi84k--6-WdgJp-H6kvKO_IsueAHX\toShareWithJonBoulanger\data';
[filename,pathname] = uigetfile({'*.tif';'*.TIFF'},'Open TIFF file with imaging video', 'MultiSelect', 'off',default_pathname);
...
while end_pro==0
    switch step
        ...
        case 'Select_Red'
            ...
            %%%%%%%%%%%%% STEP 8: Select red fluorescent cell %%%%%%%%%%%%%%%%%%            
            hf=figure(200); clf; hold on
            
            set(gcf,'Name','Select red cell', 'Toolbar','figure');
            hGam=uicontrol('Style','slider','String','Set Gamma','position',[scrsz(3)*8/10 scrsz(4)*3/10 120 60],'Min', 0.01,'Max',2,...
                'value', valGam,'SliderStep',[.01 .05],'Callback','[myColors valGam]=changeGammaAdrien(hGam,myColorsOrig);');
            uicontrol('Style','pushbutton','String','Done!','CallBack','end_pro=1;','Position',[scrsz(3)*8/10 scrsz(4)*2/10 120 60]);
            uicontrol('Style','pushbutton','String','AddRed','CallBack','AddRedRoi=1;','Position',[scrsz(3)*8/10 scrsz(4)*5/10 120 60]);
            AddRedRoi=0;
            ROIRed=[];
            
            imagesc(avg); 
            colormap(myColors);  axis image; hold on
            set(gca,'xticklabel','','yticklabel','')
            for i=1:length(cell_per)
                lp=line(cell_per{i}(:,1),cell_per{i}(:,2));
                set(lp,'color',[ismember(i,ROIRed) ~ismember(i,ROIRed) 0],'LineWidth',0.5);
            end
            
            drawnow
            while end_pro==0
                colormap(myColors);
                uicontrol('Style','text','Position',[scrsz(3)*8/10 scrsz(4)*3.7/10 120 30],'String',{'Gamma' valGam})
                
                drawnow
                
                if AddRedRoi==1
                    [colToDel, rowToDel] = getpts(gcf);
                    indROIRed=sub2ind(size(avg),round(rowToDel),round(colToDel));
                    if ismember(indROIRed, find(bkg))
                        for j=1:length(cell_per)
                            if ismember(indROIRed,cells{j})
                                if ~ismember(j,ROIRed)
                                    ROIRed=[ROIRed,j];
                                    break
                                end
                            end
...
save(outputFile,'avg','bkg','cell_number','cells_mean','npil_mean','cell_per','cells','centers','distances','pixelLengthX','pixelLengthY','ROIRed');
